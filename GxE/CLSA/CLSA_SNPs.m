...
for i=1:215; i
    ...
    mdl=fitlm(T, 'COG_REYII_SCORE ~ SNP + AGE_BL + SEX_ASK +SEX_ASK*AGE_BL+ PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10');
    snp_stats_main(i,:)=mdl.Coefficients(strcmp(mdl.Coefficients.Properties.RowNames,'SNP'),:);
    mdl=fitlm(T, 'COG_REYII_SCORE ~ SNP*hypert + AGE_BL + SEX_ASK +SEX_ASK*AGE_BL+ PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10');
    snp_stats_int(i,:)=mdl.Coefficients(strcmp(mdl.Coefficients.Properties.RowNames,'hypert:SNP'),:);
    mdl=fitlm(T, 'COG_REYII_SCORE ~ SNP*DEP + AGE_BL + SEX_ASK +SEX_ASK*AGE_BL+ PC1+PC2+PC3+PC4+PC5+PC6+PC7+PC8+PC9+PC10');
